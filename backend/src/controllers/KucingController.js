...
class KucingController {
  ...
  async getPelamarByKucing(request, reply) {
    try {
      ...
      // Memanggil fungsi yang baru kita buat di Model
  ...
  async updateStatusPelamar(request, reply) {
    try {
      const { id } = request.params; // Ini ID dari aplikasi_adopsi
      const { status } = request.body; // Isinya 'disetujui' atau 'ditolak'
      
      // 1. Update status pelamar di tabel 'aplikasi_adopsi'
      await this.kucingModel.updateStatusPelamar(id, status);
      
      // 2. LOGIKA TAMBAHAN: Jika statusnya 'disetujui' (diterima)
      if (status === 'disetujui') {
        // Cari tahu ID kucing yang dilamar
        const kucingId = await this.kucingModel.getKucingIdByPelamar(id);
        
        if (kucingId) {
          // Ubah status kucing tersebut jadi 'teradopsi'
          // (Gunakan nama 'updateStatusKucinag' sesuai yang kamu tulis di Model tadi, 
          // atau perbaiki typo-nya di Model menjadi 'updateStatusKucing')
          await this.kucingModel.updateStatusKucinag(kucingId, 'teradopsi');
        }
      }
      
      return reply.send({ 
        success: true, 
        message: `Pelamar berhasil ${status === 'disetujui' ? 'disetujui' : 'ditolak'}` 
      });
    } catch (error) {
      console.error("Error updateStatusPelamar:", error);
      return reply.status(500).send({ success: false, error: error.message });
    }
  }
...
async getKucingSaya(request, reply) {
  try {
    ...
    // Memanggil fungsi findByOwner yang sudah ada di KucingModel
    const results = await this.kucingModel.findByOwner(userId);
    
    return reply.send({ success: true, data: results });
  } catch (error) {
    console.error("Error getKucingSaya:", error);
    return reply.status(500).send({ success: false, error: error.message });
  }
}
