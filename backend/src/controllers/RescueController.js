...
async createRescue(request, reply) {
 ...
 async updateStatus(request, reply) {
  ...
  // Todong langsung listriknya dari server
  const db = request.server.db; 

  try {
    // Eksekusi update
    await db.query("UPDATE laporan_rescue SET status = ? WHERE id = ?", [status, id]);
    
    // Kasih jawaban ke HP kalau berhasil
    return { success: true, message: "Yess! Status kucingnya berubah!" };
  } catch (error) {
    return reply.status(500).send({ success: false, message: "Yah, gagal update ke database" });
  }
}
