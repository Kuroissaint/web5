...
class PengajuanAdopsiController {
    ...
    async createPengajuanAdopsi(request, reply) {
      ...
      try {
          ...
          const newKucingId = resKucing.insertId; // ID Kucing didapat!
            ...
            // --- Perbaikan di pengajuanAdopsiController.js ---
    // Ganti bagian handling foto (Langkah 2) menjadi:
    ...
    if (parts.foto) {
        ...
        for (const file of files) {
            ...
            // Simpan file fisik (Pakai Buffer dari Fastify Multipart)
            await fs.writeFile(savePath, file._buf || await file.toBuffer());
    ...
    async getPengajuanByUserId(request, reply) {
        try {
            ...
            console.log("Mencari pengajuan untuk User ID:", userId); // Untuk cek di terminal
