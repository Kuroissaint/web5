...
class KucingModel {
  ...
  async findAll() {
    const [rows] = await this.db.execute(
        `SELECT 
            ...
            GROUP_CONCAT(g.url_gambar) as url_gambar
            FROM kucing k
    ...
    // Fungsi Detail khusus untuk bagian ADOPSI kamu
    // 1. Perbaikan fungsi findById (Detail Adopsi)
    async findById(id) {
        try {
            const [rows] = await this.db.execute(
                `SELECT 
                    k.id, k.nama_kucing, k.umur, k.jenis_kelamin, k.warna_bulu, k.deskripsi, k.sudah_steril, k.status,
                    pa.alamat_lengkap, pa.telepon as no_telepon_pemilik, pa.nama_lengkap as nama_pemilik,
                    p.nama_provinsi, kk.nama_kabupaten_kota, kc.nama_kecamatan,
                    (SELECT GROUP_CONCAT(url_gambar) FROM gambar WHERE entitas_id = k.id AND jenis_entitas = 'kucing') as url_gambar
                FROM kucing k
                -- Gunakan LEFT JOIN agar kucing tetap muncul meskipun data adopsinya belum lengkap
            ...
            console.error("Error di findById:", error);
            throw error;
        }
    }

    // 2. Perbaikan fungsi getPengajuanByKucingId (Isi SQL-nya)
    async getPengajuanByKucingId(kucing_id, connection) {
        const runner = connection || this.db;
        try {
            const [rows] = await runner.execute(
                `SELECT * FROM pengajuan_adopsi WHERE kucing_id = ?`,
                [kucing_id]
            );
            return rows[0] || null;
        } catch (error) {
            console.error("Error di getPengajuanByKucingId:", error);
            throw error;
        }
    }
  ...
  async createLaporan(data) {
      let connection; 
      try {
          // Gunakan getConnection dari pool yang sudah di-inject (this.db)
          ...
          const [kucingResult] = await connection.execute(
              `INSERT INTO kucing (nama_kucing, jenis_kucing_id, deskripsi, status,dibuat_oleh, created_at) 
               VALUES (?, ?, ?, 'hilang', NOW())`,
              [data.nama_kucing, jenisKucingId, data.deskripsi, data.userId]
          );
          const newKucingId = kucingResult.insertId;

          // C. Insert Laporan Hilang
  ...
  async createKucing(data) {
      ...
      const params = [
          ...
          data.dibuat_oleh, data.created_at, data.updated_at
      ];
  ...
  // Fungsi untuk mengambil data orang yang mau adopsi
    async getPelamarByKucing(kucingId) {
    const [rows] = await this.db.execute(
        `SELECT 
        id, 
        nama_lengkap, 
        telepon,                -- Untuk dihubungi pemilik
        pekerjaan, 
        umur,                   -- Untuk melihat kematangan pelamar
        alamat_lengkap,         -- Untuk melihat lingkungan rumah
        pernah_pelihara_kucing, -- Untuk melihat pengalaman
        alasan_adopsi, 
        status, 
        created_at 
        FROM aplikasi_adopsi 
        WHERE kucing_id = ?`, 
        [kucingId]
    );
    return rows;
    ...
    // --- TAMBAHKAN KODE INI DI BAWAH getPelamarByKucing ---

  // 1. Fungsi untuk update status pelamar (Setuju/Tolak)
  ...
  // 2. Fungsi untuk otomatis ubah status kucing jadi 'teradopsi' 
  // (Dipanggil jika pemilik klik 'Terima')
  ...
  // 3. Fungsi pembantu untuk mencari kucingId berdasarkan pelamarId
  // (Penting agar kita tahu kucing mana yang harus diubah statusnya jadi 'teradopsi')
  ...
  // Fungsi untuk mengambil kucing milik user tertentu (Aktivitas Saya)
  async findByOwner(userId) {
    const query = `
      SELECT 
        k.id, k.nama_kucing, k.status,
