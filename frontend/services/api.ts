...
export const wilayahAPI = {
  getProvinsi: () => api.get('/wilayah/provinces'),
  getKabupaten: (id: string) => api.get(`/wilayah/cities/${id}`),
  getKecamatan: (id: string) => api.get(`/wilayah/districts/${id}`),
};
...
export const ChatService = {
  // 1. Fungsi untuk list inbox (Harus ada!)
  getConversations: (userId: number | string) => 
    api.get(`/chat/conversations/${userId}`),

  // 2. Fungsi untuk isi pesan
  getMessages: (id_percakapan: string) => 
    api.get(`/chat/messages/${id_percakapan}`),

  // 3. Fungsi untuk kirim pesan
  saveMessage: (data: any) => 
    api.post('/chat/send', data),

  markAsRead: (id_percakapan: string) => 
    api.put(`/chat/read/${id_percakapan}`),
};

export const BASE_URL = 'http://192.168.1.44:3000'; 
